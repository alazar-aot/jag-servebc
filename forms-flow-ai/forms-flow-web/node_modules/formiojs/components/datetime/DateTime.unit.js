...
describe('DateTime Component', function () {
  ...
  it('Should update value if time mode is on when OnBlur event triggered', function (done) {
    var form = _lodash.default.cloneDeep(_fixtures.comp4);

    var element = document.createElement('div');

    _Formio.default.createForm(element, form).then(function (form) {
      var dateTime = form.getComponent('time');
      var calendar = dateTime.element.querySelector('.flatpickr-input').widget.calendar;
      var blurEvent = new Event('blur');
      ...
      setTimeout(function () {
        _powerAssert.default.equal(dateTime.dataValue !== '', true);
